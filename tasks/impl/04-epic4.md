...
|---|---|
| `src/main/java/com/trawhile/service/ReportService.java` | `getReport(filters, mode)` SR-037/038; `getMemberSummaries(filters)` SR-063 |
| `src/main/java/com/trawhile/service/ReportExportService.java` | `exportCsv(filters)` SR-039 |
...
- **SR-037 node filter**: when supplied, include all visible nodes in that subtree
- **SR-063 granularity**: individual `started_at`/`ended_at` per entry must never appear in member summary responses
- **SR-063 bucketing**: use company timezone from `TrawhileConfig.timezone()` for interval boundaries

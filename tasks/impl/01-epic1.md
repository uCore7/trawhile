...
- **SR-F047.F02** called by four SRs (SR-F007.F01, SR-C010.C01, SR-F008.F01, SR-F047.F01) — implement once as `private scrubUser(UUID)` in `UserService`; `AccountService.anonymizeAccount()` (impl/06) delegates here too
- **SR-F047.F02 stub retention**: delete `users` row only if NO `time_entries` AND NO `requests` reference it
- **SR-F047.F02** must set `ended_at = NOW()` on active `time_entries` before deleting `user_profile`

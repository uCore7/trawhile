...
| TE-F050.F02-01 | Insert a `time_records` row with `started_at = NOW() - retentionYears YEARS - INTERVAL '1 day'` (before cutoff) and one with `started_at = NOW() - retentionYears YEARS + INTERVAL '1 day'` (after cutoff); also insert a `requests` row before the cutoff; run the activity purge job; before-cutoff rows deleted; after-cutoff row retained |
...
| TE-F050.F04-01 | Create a deactivated leaf node with `deactivated_at` before the node purge cutoff and no remaining `time_records` or `requests`; run the node purge job; node deleted. Also create a deactivated leaf node with a remaining `time_records` row; node retained after the job |

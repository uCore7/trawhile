...
src/test/java/com/trawhile/
  ...
  TimeEntryIT.java    — TE-F031.F01-01/02/03, TE-F032.F01-01/02, TE-F033.F01-01, TE-F034.F01-01
```
...
| TE-F024.F01-02 | `GET /api/v1/tracking` when not tracking: response indicates empty/no active entry |
| TE-F025.F01-01 | Insert two overlapping `time_entries`; `GET /api/v1/time-entries` (recent): both entries have `overlap: true` |
| TE-F025.F01-02 | Insert two consecutive entries with a gap; `GET /api/v1/time-entries` (recent): gap flag set on the pair |
| TE-F026.F01-01 | `POST /api/v1/tracking` on active leaf node with `track` auth: `SELECT COUNT(*) FROM time_entries WHERE ended_at IS NULL AND user_id = {id}` = 1; 409 on inactive node |
| TE-F026.F01-02 | `POST /api/v1/tracking` on node with active children: 409 |
| TE-F026.F01-03 | `POST /api/v1/tracking` without `track` auth: 403 |
| TE-F028.F01-01 | `POST /api/v1/tracking` while already tracking a different node: old entry has `ended_at IS NOT NULL`; new entry has `ended_at IS NULL`; only 1 open entry total |
| TE-F029.F01-01 | `DELETE /api/v1/tracking` while tracking: entry gets `ended_at IS NOT NULL`; `GET /api/v1/tracking` returns empty state; 400 when not tracking |
...
| TE-F031.F01-01 | `POST /api/v1/time-entries` with valid `started_at < ended_at`: row inserted with correct `timezone`; 400 when `started_at >= ended_at` |
| TE-F031.F01-02 | Create retroactive entry on a node without `track` auth: 403 |
| TE-F031.F01-03 | Create retroactive entry on inactive node: 409 |
| TE-F032.F01-01 | `PATCH /api/v1/time-entries/{id}` within freeze cutoff: DB row updated; 409 when `started_at` is before `NOW() - freeze_offset_years YEARS` |
| TE-F032.F01-02 | Edit with `started_at >= ended_at`: 400 |
| TE-F033.F01-01 | `DELETE /api/v1/time-entries/{id}` not frozen: row deleted; 409 when frozen |
| TE-F034.F01-01 | `POST /api/v1/time-entries/{id}/duplicate` with new times: new row has same `description` as original, new `started_at`/`ended_at`; 400 when new `started_at >= ended_at` |

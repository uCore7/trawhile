...
4. `docs/test-plan.md` — TE-049-* through TE-051-*
...
src/test/java/com/trawhile/
  SecurityEventIT.java    — TE-049-01/02/03/04, TE-050-01, TE-051-01
```
...
| TE | What to assert |
|---|---|
| TE-049-01 | Trigger a successful login flow; `SELECT COUNT(*) FROM security_events WHERE event_type = 'LOGIN_SUCCESS'` increases by 1 |
| TE-049-02 | Perform a grant authorization operation; `SELECT COUNT(*) FROM security_events WHERE event_type = 'AUTH_GRANT'` increases by 1 |
| TE-049-03 | Generate an MCP token; `SELECT COUNT(*) FROM security_events WHERE event_type = 'MCP_TOKEN_GENERATED'` increases by 1 |
| TE-049-04 | Revoke an MCP token; `SELECT COUNT(*) FROM security_events WHERE event_type = 'MCP_TOKEN_REVOKED'` increases by 1 |
| TE-050-01 | `GET /api/v1/security-events` as root admin with filter params: response is a list; filtering by `event_type` returns only matching rows; 403 for non-admin |
| TE-051-01 | Insert a `security_events` row with `occurred_at = NOW() - INTERVAL '91 days'` and one with `occurred_at = NOW() - INTERVAL '89 days'`; run the cleanup method; 91-day row deleted; 89-day row retained |
